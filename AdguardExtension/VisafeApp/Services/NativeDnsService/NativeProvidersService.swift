...
class NativeProvidersService: NativeProvidersServiceProtocol {
    ...
    private lazy var VisafeDnsServer: DnsServerInfo? = {
        let VisafeDnsProviderId = 10001
        let VisafeProvider = providers.first(where: { $0.providerId == VisafeDnsProviderId })
